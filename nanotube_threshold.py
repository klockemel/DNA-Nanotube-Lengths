...
'''This script contains the entire workflow for getting length measurements from raw .nd2 nikon images of nanotubes.
The process is inspired by the previous method used by Franco lab to measure lengths, including a script written by 
Dr. Vahid Mardanlou in MATLAB which took in thresholded binarized .tif image of nanotubes, removed branching or intersecting
features, and measured the lengths of the remaining features in the image. The script by Dr. Mardanlou was called "TubeRemove_Vahid3.m
The "subtractBackground" function can be bypassed if the images are high quality - no background junk, even illumination across field of view, 
bright nanotubes compared to background, etc. The script does run more slowly with background removal step, but allows for processing images
with some of the issues mentioned in the previous sentence.
...
def main():
	...
	fn = sys.argv[1] 
	median_size = 10	#Recommend 10. Can try 15, 20 for images with bad backgrounds. Increased values caused the code to run noticeably more slowly
	sigma_val = 2 #can be adjusted from 0.5-4 depending on the quality of the images. Larger values result in more smoothing
	...
	thresh = 'otsu' ## or 'yen' or 'otsu' or 'tri'

	os.system("mkdir _figs")
	print("\nOpening file: ", fn)

	fname, pix_micron, img = nd2_read(fn) 
	...
	saveRunValues(fn, fname, median_size, sigma_val, thresh, removeSmallerThan)
...
def threshold(img, sigma, thresh, fn):
	...
	if thresh == 'otsu':
		...
		thresh_val = filters.threshold_triangle(gauss_filtered_img)
...
def branchpoints(skeleton, fn):
	...
	with Vahid's Matlab script into one neat python script. The function applies the hit_or_miss function from scipy.ndimage using 
	selems which represent different kinds of intersections between linear features.'''
	...
	selems = [np.rot90(selems[i], k=j) for i in range(6) for j in range(5)]
...
def saveRunValues(fn, fname, med, sigma, thresh, minSize):
	'''Saves the user-input parameters to a csv file for reproducibility and diagnostic purposes.'''
	dict_vals = {'filename': [fn], 'median smoothing value': [med],'smoothing sigma value': [sigma], 'threshold type': [thresh], 
	'objects smaller then n (pix) removed': minSize}
